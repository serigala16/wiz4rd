...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome HITS SSH${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/serigala16/wiz4rd/MASTER/ip_vps | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/serigala16/wiz4rd/MASTER/ip_vps | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/serigala16/wiz4rd/MASTER/ip_vps | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/serigala16/wiz4rd/MASTER/ip_vps | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/serigala16/wiz4rd/MASTER/ip_vps | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/88PanelSc/wiz4rd/MONSTER/limit/limit.sh && chmod +x limit.sh && ./limit.sh
